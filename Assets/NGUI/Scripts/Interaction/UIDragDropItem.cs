...
#region Common functionality
	...
	{
		var v = MultiplyQV(this.transform.rotation, delta);
		//mTrans.localPosition += mTrans.InverseTransformDirection((Vector3)delta);
		mTrans.localPosition += mTrans.InverseTransformDirection(v);
		OnProcessDrag?.Invoke();
	}

	public Vector3 MultiplyQV(Quaternion quat, Vector3 vec)
	{
		float num = quat.x * 2f;
